...
Στην παρούσα ενότητα θα παρουσιαστεί ένα σύνολο σεναρίων που συνδέονται με ένα συγκεκριμένο σκοπό του χρήστη.Αξίζει να σημειωθεί ότι δεν 
αναλύονται όλες οι λειτουργίες του συστήματος παρά μόνο οι σημαντικότερες.


Ορισμένα από τα σενάρια χρήσης θα αναπαρασταθούν σε μορφή διαγράμματος *Use Case*. Τα διαγράμματα περιπτώσεων χρήσεων (*Use Case 
Diagrams*) περιγράφουν τις λειτουργικές απαιτήσεις του συστήματος. Μία περίπτωση χρήσης (*Use Case*) μπορεί να είναι ένα σύνολο σεναρίων 
με τον ίδιο στόχο είτε και ένα σύνολο σεναρίων που αποσκοπούν στην εκπλήρωση ενός στόχου ενός χρήστη.cΟι χρήστες ονομάζονται χειριστές 
(*actors*), και επισημαίνεται ότι δεν είναι απαραίτητα άνθρωποι. Επιπλέον κάθε εξωτερικό σύστηµα που αλληλεπιδρά µε το υπό ανάπτυξη 
σύστηµα αναπαρίσταται µε ένα χειριστή. Μία περίπτωση χρήσης μπορεί να περιλαμβάνει μια άλλη (*includes*), ή και να επεκτείνει μια άλλη 
(*extends*)
Παρακάτω παρουσίαζεται το διάγραμμα χρήσης του Bizard. Στο παράδειγµά µας του ηλεκτρονικού ανθοπωλείου έχουµε εντοπίσει τους
παρακάτω χειριστές:
* Πελάτης
* Καταχωρημένοι Πελάτες
* Επισκέπτες
...
Οι χειριστές «πελάτης» µε τους χειριστές «καταχωρηµένος πελάτης» και «Επισκέπτες» συνδέονται µε σχέση γενίκευσης. Η σχέση γενίκευσης 
χρησιµοποιείται µεταξύ χειριστών όταν αυτοί µοιράζονται κοινές χρήσεις και κοινή συµπεριφορά.  
...
Συνήθως χρησιµοποιούνται για την καλύτερη περιγραφή των περιπτώσεων χρήσης, την περιγραφή της ροής των εργασιών (workflow), την περιγραφή ενός πολύπλοκου αλγορίθµου και την περιγραφή παράλληλων δραστηριοτήτων. Αξίζει, λοιπόν, να σημειωθεί ότι τα διαγράµµατα δραστηριοτήτων δε µπορούν να χρησιµοποιηθούν για την περιγραφή του τρόπου συνεργασίας των αντικειµένων ή της εσωτερικής συµπεριφοράς των αντικειµένων.
Στη µελέτη περίπτωσης του e-shop *Bizart*, και στην παρούσα φάση ανάπτυξης τα διαγράµµατα δραστηριοτήτων θα µπορούσαν να χρησιµοποιηθούν για την καλύτερη κατανόηση των περιπτώσεων χρήσης.

Παρακάτω δίνονται πέντε διαγράμματα δραστηριοτήτων:
![Screenshot](uml_Activity_1.png)
Στο διάγραµµα αυτό παρουσιάζονται τα επιµέρους βήµατα που εκτελούνται για την αγορά ενός πίνακα από καταχωρημένους χρήστες-πελάτες. 
...
αυτών αχρηστεύει το σύστηµα.Πιο συγκεκριμένα αποτελόυν τους περιορισμούς στις υπηρεσίες ή τις λειτουργίες που προσφέρει το συστήμα, όπως 
χρονικοί περιορισμοί, περιορισμοό στην διαδικασία ανάπτυξης, αποδοτικότητα, πρότυπα, την απόδοση, την νομιμότητα και την ιδιωτικότητα 
κτλ. Οι μη λειτουργικές απαιτήσεις πρέπει να είναι μετρήσιμες. Παρακάτω παρπυσιάζονται ορισμένες από τις μη λειτουργικές απαιτήσεις του 
e-shop.
...
* **Περιγραφή:** Η συγκεκριμένη απαίτηση κατατάσεται στα παλαίσια της γρήγορης εκτέλεσης διαδικασιών από το σύστημα. Για να ολοκληρωθούν οι αγορές απαιτήται η εισαγωγή του αντίστοιχου χρηματικού ποσού από τον πελάτη οοποίος θα εισάγει τον αριθμό πιστωτικής του κάρτας και το ποσό. Συνεπώς ο χρόνος έγγρισης της καρτας δεν πρέπει να υπερβαίνει τα 5 δευτερόλεπτα.

* **User Priority: (5/5):** Ουσιώδης για τον χρήστη καθώς η γρήγορη διεκπαιρέωση της συγκεκριμένης διαδικασίας διευκολίνει την συνναλαγή.

* **Technical Priority: (5/5)** Η ομάδα ανάπτυξης θα πρέπει να λάβει υπόψην τον χρόνο υλοποίησης της λειτουργίας καθώς γρήγορες συνναλαγές συμβάλουν στην ανάπτυξη των πωλήσεων.

* **Stability: (4/5)** Ο χρόνος επικοινωνίας εξαρτάταται από το εξωτερικό σύστημα έγκρισης χρήσης πιστωτικών καρτών, συνεπώς μπορεί να υπάρξουν αλλαγές ως προς τον χρόνο, για παράδειγμα αντί για 5 δευτερόλεπτα μπορεί να υλοποιείται σε 3 ή και 7.
...
### ΜΛΑ-11
* Ο χρόνος καταχώρισης της παραγγελίας στο Καλάθι Αγορών δεν πρέπει να ξεπερνά τα 3 δευτερόλεπτα. 
